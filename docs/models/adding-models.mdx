...
3. In **Model**, type to search the live catalog available to that connection
   and pick a model or OpenRouter saved preset. For OpenAI, OAO calls
   `GET /v1/models` with the selected connection's key and lists the returned
   models that its Responses runtime supports. The picker shows the first
...
options. For direct OpenAI connections, OAO decrypts the selected project key
server-side and fetches OpenAI's live `/v1/models` catalog. The response is
intersected with OAO's supported Responses models so embeddings, image, audio,
and other incompatible model types are not offered for agent presets. Catalog
responses contain public metadata only; neither provider key is returned.
...
If a direct OpenAI model is missing, confirm the selected key can list it through
OpenAI's `/v1/models` endpoint. OAO also requires matching Responses-runtime
metadata, so a newly released or non-text model remains unavailable until the
pinned Pi catalog supports it.
