...
description: Configure project-scoped OpenRouter, OpenAI, or Anthropic credentials and link approved models to agents.
---

OAO stores hosted model configuration in PostgreSQL per organization and
project. The MVP supports three provider types: **OpenRouter**, **OpenAI**, and
**Anthropic**.
...
Do not put `OPENROUTER_API_KEY`, `OPENAI_API_KEY`, or `ANTHROPIC_API_KEY` in
`.env`. Those keys are entered through the project UI or API and stored as
tenant-scoped ciphertext.
...
3. Choose **OpenRouter**, **OpenAI**, or **Anthropic**.
...
3. In **Model**, type to search the live catalog available to that connection
   and pick a model or OpenRouter saved preset. For direct providers, OAO calls
   their authenticated `GET /v1/models` endpoint and lists returned models that
   the corresponding runtime supports. The picker shows the first
...
and price caps. OpenAI and Anthropic are direct provider connections, so
...
Each response is intersected with OAO's pinned runtime models so unsupported
model types are not offered for agent presets. Anthropic entries also expose
...
OpenAI, and Anthropic use separate catalogs and prefixes. If an OpenRouter model
