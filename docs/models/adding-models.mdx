...
description: Configure organization-shared OpenRouter, OpenAI, Anthropic, or xAI credentials and link approved models to agents.
---
...
live list for account membership and accepts new GPT-4.1, GPT-5-and-later, and
o3-and-later text/reasoning family IDs without a pinned catalog update. Known
...
Family names are an eligibility check, not a provider capability guarantee;
OpenAI still validates actual endpoint and feature support when a run starts.
...
image input, pricing, and reasoning levels. Models without metadata show
unknown context/output limits in the catalog. Their runtime uses text input,
a 32,768-token context budget, and a 4,096-token output budget. These are local
fallback budgets, not verified provider limits. Local cost estimates are zero
until pricing metadata is available; this does not mean the provider is free.
New model families or specialized endpoints may still require adapter support.
