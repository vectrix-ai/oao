...
Runtime execution requires a configured project model preset and Daytona
connection. There is no selectable fake model or sandbox profile.
...
<Steps>
  <Step title="Create the definition">
    ...
    policy, optionally select an active Daytona snapshot, and select exact tool
    capabilities before submitting the form.
  ...
  <Step title="Review and publish a new version">
    ...
    approved model preset, sandbox switch, network policy, or timeout. The
    searchable model list is the project's approved preset catalog, and the
    editor shows the resolved model and routing policy for the selected preset
    with a link to **Manage models**. Select **Publish new version** to append an immutable
    version.

    Linking a different model is always a publication. Existing sessions stay
    attached to the version they were created with.
  ...
  <Step title="Create a session">
    ...
    enter a title and first message, and submit.

    The session pins the agent's latest version at that moment. Creation returns
    while the first run is `queued`; the runtime finishes it asynchronously.
  ...
  <Step title="Continue after settlement">
    ...
    creates a new durable run on the existing session and thread.
  </Step>
</Steps>

<Warning>
  The **Add tool** control is not wired up in the current console. Existing
  tools are displayed, but publish tool definitions through the API. The current
  API-key, archive, rename, resume, and branch-replay controls are also not
...
| `inputSchema`   | Closed supported object schema             |
| `outputSchema`  | Closed supported object schema             |

Both root schemas must be objects with `properties`, `required`, and
`additionalProperties: false`. Nested schemas support only:

- Primitive `{ "type": "string" | "number" | "integer" | "boolean" | "null" }`.
- Arrays with exactly `type` and `items`.
- Closed objects with exactly `type`, `properties`, `required`, and
  `additionalProperties`.
- Non-empty primitive `enum` arrays represented by an object containing only
  `enum`.

Keywords such as `description`, `minLength`, `maximum`, `pattern`, `format`,
`oneOf`, `$ref`, and open-ended properties are rejected rather than silently
ignored. Every name in `required` must be unique and present in `properties`.
...
`daytona-primary`.
`snapshotId` is an optional active snapshot UUID returned by that provider's
snapshot discovery route. The selected UUID is immutable version data; omit it
to use OAO's default `debian:12.9` image.
...
target selection. Those settings come from the selected Daytona snapshot or
the adapter default.
...
- **Transcript** for user, assistant, reasoning, and tool-facing records. User and
  assistant messages render CommonMark plus GitHub Flavored Markdown, including
  lists, emphasis, links, code, task lists, and tables. Raw HTML is not rendered.
...
<Warning>
  The current console's caller-result form sends the entered object directly as
  `safeResult`; it does not add the required `version`/`status`/`value`
  envelope. Use the API for caller results until that UI is completed.
</Warning>
...
- The console can edit instructions, model preset, sandbox provider, sandbox
  ...
  latest version. It cannot author caller/platform tools.
