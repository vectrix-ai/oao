...
<Steps>
  ...
  <Step title="Review and publish a new version">
    ...
    approved model preset, bound Skill versions, exact delegate roster, sandbox switch, network policy, or timeout. The
    ...
    Linking a different model or Skill version is always a publication.
...
complete SDK/API examples. A session or turn may omit its text field only when
