...
- **Run files are deliberately bounded.** Initial and later turns accept up to
  ...
  Office/OpenDocument/iWork documents, or EML/MSG emails. Extracted content is
  capped at 200,000 characters per turn. Password-protected or text-free
  documents, standalone archives, audio, video, Access/Publisher/OneNote/Visio/
  Project files, and arbitrary binaries are not accepted. Files are submitted
  inline as base64; presigned/direct-to-object storage uploads and reusable
  artifact references remain future work. For full codebases, use narrow
  caller-owned read/search tools instead of attaching the repository.
