...
For automated setup, humans and services need the same organization membership
and `project:admin` (or `*`). An existing API key qualifies through `project:admin`
or `*`. Every creator can grant only scopes they already hold; `*` permits any
...
revoke versions. Delegation inspection, messaging, and cancellation use
`delegation:read`, `delegation:message`, and `delegation:cancel` respectively.
