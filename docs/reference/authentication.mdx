...
When the console has no session, it first tries the development-login endpoint.
If that endpoint is unavailable in WorkOS mode, it calls `POST /v1/auth/login`
and navigates to the HTTPS `redirectUrl` returned by the API. The callback then
sets the HTTP-only session cookies and redirects back to the configured app
origin. While that navigation is in progress, concurrent console requests reuse
the same login attempt so they cannot replace its one-time state cookie.
