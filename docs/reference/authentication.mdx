...
- **Google IAP** verifies the signed assertion injected at the application edge
  and maps its immutable subject to an explicitly provisioned OAO principal.
...
OAO maps the signed `sub` to an existing principal, membership, and scopes. A
valid Google identity alone never creates a tenant or grants OAO access.

Set `AUTH_PROVIDER=iap` with the exact Cloud Run resource audience and default
OAO tenant. Before first login, run `provision:iap` as documented in
`apps/api/README.md`. The command may pre-authorize the signed email when the
IAP `sub` is not known yet; the first matching assertion binds that immutable
subject without creating or promoting a principal.
...
console. Enter a descriptive name, select the least-privilege scopes, and save
the secret from the acknowledgement dialog. The console removes the plaintext
