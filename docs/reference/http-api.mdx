---
title: HTTP API
description: Use the project and organization endpoints, pagination, idempotency, and error contracts.
---
...
identity records as the rejoining guard. `POST /members` is disabled in IAP mode;
users join through verified IAP login. Other authentication modes retain the
existing project-role and explicitly supplied scope behavior.

All routes require `project:admin`. Member and project writes also require an
`Idempotency-Key`. Member create input is `{ subject, role, scopes }`; roles
are `owner`, `admin`, `member`, or `viewer`. A member response includes the
...
remaining project of an organization cannot be deleted. Organization-shared
resources (API keys, model, storage, sandbox, and MCP connections) are
untouched.
