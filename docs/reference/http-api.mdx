...
and results directly as formatted, highlighted JSON, without a second payload
inspector. Caller tool calls from `debug.toolCalls` expand the same way and
show their recorded `safeArguments`; tool events that carry only other safe
metadata (for example Skill activations) expand to that metadata instead.
