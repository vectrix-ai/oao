...
session creation. Message text is limited to 100,000 characters and decoded or
extracted file context to 200,000 characters per turn. A text field may be
omitted only when `files` is non-empty. See
...
and base64 are never returned from those endpoints.
