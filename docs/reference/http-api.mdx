...
selected key that OAO's Responses runtime supports. OpenAI presets require an
empty routing object and accept the immutable OpenAI `settings` object below.
...
(`400`), stays in the list with a non-null `disabledAt`, and is not offered to
runs of agent versions that already bind it. Enabling restores both. Repeating
the current state is a no-op that still returns `200`. The audit log and
product events record `skill.disabled` and `skill.enabled` only when the state
changed.
...
bindings are immutable and stay stored. After removal the Skill returns `404`
from reads and version publication, disappears from the list, cannot be
attached to a new agent version, is not offered to runs of agent versions that
already bind it, and its open drafts are discarded. Its `key` becomes free for a
new Skill. Removal cannot be undone. The audit log and product events record
`skill.deleted`. Removing an already removed Skill returns `404`.
