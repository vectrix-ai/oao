...
window, output limit, and reasoning support. Anthropic entries may additionally
include `adaptiveThinking`, `thinkingCanBeDisabled`, and `effortLevels` from
the live model capabilities.
...
`xai/<catalog-id>`. xAI presets require empty routing and `settings: null` (or
an omitted `settings` field). Newly listed language-model IDs do not require a
hard-coded OAO catalog update.
