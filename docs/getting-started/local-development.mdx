...
OAO's default development profile runs the console, API, runtime worker, and PostgreSQL on your machine. Development authentication is local, but executing an agent requires an OpenRouter, OpenAI, or Anthropic connection.
...
### OpenRouter, OpenAI, and Anthropic models
...
Restart the API and runtime, then add an OpenRouter, OpenAI, or Anthropic provider on the
...
the returned models supported by OAO's pinned native runtimes. No process restart is
...
output-token ceiling, and a supported effort level. The runtime applies these
settings to every session using that preset.

<Warning>
  Do not set `OPENROUTER_API_KEY`, `OPENAI_API_KEY`, or `ANTHROPIC_API_KEY` in
  `.env`. Provider keys are write-only console/API inputs and are never returned
  after encryption.
</Warning>
