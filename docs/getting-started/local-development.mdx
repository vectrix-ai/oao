...
catalog available to the selected provider key; for OpenAI it offers the
returned models supported by OAO's Responses runtime. No process restart is
needed for later provider or preset additions. Direct OpenAI presets also pin
plain-text output, reasoning mode and effort, verbosity, and reasoning-summary
settings; the runtime applies them to every OpenAI Responses session using that
preset.
