...
OAO's default development profile runs the console, API, runtime worker, and PostgreSQL on your machine. It uses deterministic model, sandbox, and authentication adapters, so the first start does not require hosted-provider credentials.

<Info>
  PostgreSQL is the only application database in the local profile. OpenRouter,
  Daytona, WorkOS, S3-compatible storage, and an OTLP collector are not required
  for the default startup path.
</Info>
...
4. Idempotently seeds the development organization, project, and principal.
...
```text
OAO is ready at http://127.0.0.1:5173
...
| -------------- | ----------------------- | ----------------------------------------------------------- |
| Console        | `http://127.0.0.1:5173` | React management and debugging UI                           |
...
The browser should load the console without a WorkOS sign-in. The console uses the HTTP API through the Vite `/v1` proxy and the deterministic development identity.

The console starts in its light appearance. Use **Use dark appearance** in the
top bar to switch themes; the choice is stored only in the browser and does not
affect project or organization data. Both appearances preserve the same OAO
green action and status palette.
...
To deliberately reset all local OAO data, first stop the stack, then remove the named volume:

```bash
pnpm db:down
docker volume rm oao-postgres-data
```

<Warning>
  Removing `oao-postgres-data` permanently deletes the local database. The next
  `pnpm dev:local` creates a fresh volume, reapplies migrations, and reseeds
  only the deterministic development tenant.
</Warning>
...
Restart the API and runtime, then add an OpenRouter or OpenAI provider on the
...
model preset linked to that connection. No process restart is needed for later
provider or preset additions.

<Warning>
  Do not set `OPENROUTER_API_KEY` or `OPENAI_API_KEY` in `.env`. Provider keys
  are write-only console/API inputs and are never returned after encryption.
</Warning>
...
Choose the connection, an active Daytona snapshot (or OAO's default image),
...
### S3-compatible workspace backups

To preserve Daytona files after a sandbox is deleted, open **Storage
providers** and add a project-scoped S3-compatible connection. The API and
runtime use the same `OAO_CREDENTIAL_ENCRYPTION_KEY` to protect its write-only
credentials. The first connection becomes the project default automatically;
you can explicitly select another default later.
...
<Note>
  `pnpm dev:local` always starts the API with `AUTH_PROVIDER=development` and
  `NODE_ENV=development`, even if `.env` contains WorkOS values. WorkOS is a
  separate production-oriented API mode that requires HTTPS-compatible origin,
  callback, cookie, webhook, pepper, and operator-provisioning configuration. It
  is not a credential switch for the local launcher.
</Note>
...
The default ports are `5432`, `3000`, `8788`, and `5173`. Find the process holding a port:
