...
choose an active snapshot or OAO's default image, then choose `none` or
...
sandbox in the runnable product.
...
selected snapshot UUID as immutable `sandbox.snapshotId` version data. If no
snapshot is selected, creation uses `debian:12.9`.
...
When no snapshot is selected, OAO uses the public `debian:12.9` base image;
browser-capable agents therefore need a selected snapshot that adds Chromium.
