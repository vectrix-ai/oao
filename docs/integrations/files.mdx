...
dedicated [Files API reference](/reference/files-api).
...
const files = await client.listSessionFiles(projectId, sessionId);
const result = files.data.find((file) => file.path === "output/result.csv");
if (!result) throw new Error("The file was not persisted");

const download = await client.downloadSessionFile(
  projectId,
  sessionId,
  result.path,
);
await writeFile(result.name, download.bytes);
```
