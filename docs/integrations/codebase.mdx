...
<Warning>
  Direct attachment and binary upload are **not implemented**. There is no
  current endpoint that accepts a file, directory, archive, Git checkout, or
  artifact reference as session input. Do not invent an upload URL or send bytes
  to the artifact endpoints.
</Warning>

The current MVP supports two code-context patterns:

1. Put a small, trusted text excerpt directly in `initialMessage` or a later
   turn. The complete message must be no more than 100,000 JavaScript
   characters.
2. Publish narrow `caller` tools such as `read_repository_file` and
   `search_repository`, then run those tools in your own server process against
   one allowlisted repository root.

Use inline text for a deliberate, bounded snapshot. Use caller tools when the
agent needs to choose which files to inspect over several turns. Caller-tool
arguments and results are durable public records, so return only content that
...
## Pattern 1: inline a small trusted text file
...
## Pattern 2: run allowlisted repository tools
