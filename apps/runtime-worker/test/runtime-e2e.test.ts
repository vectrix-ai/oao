      ...
      if (resolution === "denied") {
        await withTenantTransaction(admin, tenant, (transaction) =>
          transaction.query(
            ...
            [tenant.organizationId, tenant.projectId, approvals[0]?.id, human],
          ),
        );
      } else {
        await admin.query(
...
if (process.argv.includes("--isolated-runtime-scenario")) {
  // This process owns the scenario's runtime and can exit even when safe
  // in-process disposal refuses to abort an active submission.
  void runRuntimeScenario().then(
    () => process.exit(0),
    (error: unknown) => {
  ...
  test(
    ...
    async (context) => {
      await runIsolatedScenario(fileURLToPath(import.meta.url), {
        args: ["--isolated-runtime-scenario"],
        timeoutMs: 180_000,
