-- Allow immutable xAI reasoning and text-format controls without changing the
-- already-applied provider and generation-settings migrations.
    ...
    AND (
      (
        (SELECT array_agg(key ORDER BY key) FROM jsonb_object_keys(p_settings) AS keys(key))
          = ARRAY['effort','mode','summary','textFormat','verbosity']::text[]
      ...
      (
        (SELECT array_agg(key ORDER BY key) FROM jsonb_object_keys(p_settings) AS keys(key))
          = ARRAY['effort','maxTokens','thinking']::text[]
      ...
      (
        (SELECT array_agg(key ORDER BY key) FROM jsonb_object_keys(p_settings) AS keys(key))
          = ARRAY['effort','textFormat']::text[]
        AND jsonb_typeof(p_settings->'textFormat') = 'string'
        AND p_settings->>'textFormat' = 'text'
        AND jsonb_typeof(p_settings->'effort') = 'string'
        AND p_settings->>'effort' IN ('low', 'medium', 'high', 'xhigh')
      )
    )
